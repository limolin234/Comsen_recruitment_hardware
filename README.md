...
1. **演示视频 / 硬件展示资料**：拍摄实物硬件，展示按键、屏幕显示与计算功能（建议控制在 1~3 分钟内）。
2. **完整可编译工程**：类似 `example/`，附带工程的 `README.md` 说明自己完成了哪些内容与模块。
3. **REPORT.md**：说明自己完成了什么、学习了什么、踩了什么坑以及有何想法。

> ⚠️ **关于 REPORT.md 的特别提醒：**  
> `REPORT.md` 必须**自己如实手写**，不求多，我们主要看你自己的真实思考与总结反馈。**不建议、也不允许用 AI 代写此报告**，如果发现是 AI 生成的千篇一律内容，将直接视同无效提交并取消资格；**其余开发、代码与调试过程鼓励充分使用 AI 辅助**。

### 提交方式

- **提交**
  - 有能力fork本仓库进行开发QQ私发我你的仓库地址.(fork仓库的话不用打包)
  - 如果对于git/github尚不熟悉就把QQ私发我压缩包.

- **整理与打包**：
  - 请清理掉不必要的编译产物（如 `build/` 中间文件），将**工程源码**、**REPORT.md** 与**演示视频**一起打包成单个压缩包（`.zip`）。
  - 压缩包体积必须**严格控制在 100MB 以内**。
  - 压缩包与邮件标题命名格式：`Comsen招新-硬件组-[年级]-[姓名]`.
